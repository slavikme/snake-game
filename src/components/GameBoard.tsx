...
type GameBoardProps = {
  columns?: number;
  rows?: number;
  options?: Pick<UseSnakeOptions, "initialIntervalMs" | "intervalReductionFactor">;
  onScoreSaved?: () => void;
};
 ...
 */
const calculateAppleScore = (eatTime: number) => Math.max(APPLE_MIN_SCORE, (1 - eatTime / APPLE_MAX_MS) * APPLE_MAX_SCORE);

const GameBoard: FC<GameBoardProps> = ({ columns = DEFAULT_COLUMNS, rows = DEFAULT_ROWS, options = {}, onScoreSaved }) => {
  const { initialIntervalMs = DEFAULT_INITIAL_INTERVAL_MS, intervalReductionFactor = DEFAULT_INTERVAL_REDUCTION_FACTOR } =
    options;
  ...
  const { reset, resume, foodPosition, speed, stepMs, getCell, paused } = useSnake(columns, rows, {
    initialyPaused: true,
    initialIntervalMs,
    ...
    onGameOver: useCallback(
      ...
      [score, setTopScore, user, applesEaten, onScoreSaved, setIsGameInProgress]
    ),
  });
      ...
      <ul className="flex gap-x-5 gap-y-1 px-2 py-1 bg-background rounded-lg flex-wrap">
        <li className="flex items-center gap-2">
          ...
          <span className="whitespace-nowrap">
            {speed} <span className="text-xs text-neutral-500">({stepMs.toFixed(0)}ms)</span>
          </span>
        </li>
        <li className="flex items-center gap-2">
          <span>Score:</span>
      ...
      >
          ...
          return (
            <div
              ...
              className={cn(
                ...
                s !== undefined && {
                  "border-l-1":
                    (s.last === "up" && (s.next === "up" || s.next === "right")) ||
                    (s.last === "down" && (s.next === "down" || s.next === "right")) ||
                    (s.last === "left" && (s.next === "up" || s.next === "down" || s.isHead)) ||
                    (s.last === "right" && s.isTail),
                  "border-r-1":
                    (s.last === "up" && (s.next === "up" || s.next === "left")) ||
                    (s.last === "down" && (s.next === "down" || s.next === "left")) ||
                    (s.last === "right" && (s.next === "up" || s.next === "down" || s.isHead)) ||
                    (s.last === "left" && s.isTail),
                  "border-t-1":
                    (s.last === "up" && (s.next === "right" || s.next === "left" || s.isHead)) ||
                    (s.last === "right" && (s.next === "right" || s.next === "down")) ||
                    (s.last === "left" && (s.next === "down" || s.next === "left")) ||
                    (s.last === "down" && s.isTail),
                  "border-b-1":
                    (s.last === "down" && (s.next === "right" || s.next === "left" || s.isHead)) ||
                    (s.last === "right" && (s.next === "up" || s.next === "right")) ||
                    (s.last === "left" && (s.next === "up" || s.next === "left")) ||
                    (s.last === "up" && s.isTail),
                  ...
                  "rounded-bl-[1px] rounded-tr-[100%]":
                    (s.last === "up" && s.next === "left") || (s.last === "right" && s.next === "down"),
                  ...
                  "rounded-br-[1px] rounded-tl-[100%]":
                    (s.last === "up" && s.next === "right") || (s.last === "left" && s.next === "down"),
                  ...
                  "rounded-tl-[1px] rounded-br-[100%]":
                    (s.last === "down" && s.next === "left") || (s.last === "right" && s.next === "up"),
                  ...
                  "rounded-tr-[1px] rounded-bl-[100%]":
                    (s.last === "down" && s.next === "right") || (s.last === "left" && s.next === "up"),
                  "rounded-bl-[100%]":
                    (s.isTail && ["up", "right"].includes(s.last)) || (s.isHead && ["down", "left"].includes(s.next)),
                  "rounded-br-[100%]":
                    (s.isTail && ["up", "left"].includes(s.last)) || (s.isHead && ["down", "right"].includes(s.next)),
                  "rounded-tl-[100%]":
                    (s.isTail && ["down", "right"].includes(s.last)) || (s.isHead && ["up", "left"].includes(s.next)),
                  "rounded-tr-[100%]":
                    (s.isTail && ["down", "left"].includes(s.last)) || (s.isHead && ["up", "right"].includes(s.next)),
                }
              )}
            />
          );
        ...
        {gameOver && (
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-background/70 z-50">
            ...
            <div className="text-sm text-neutral-500">Your score: {score.toFixed(0)}</div>
            <button
        ...
        {paused && !gameOver && (
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-background/70 z-50">
            <div className="text-2xl font-bold">{gameStarted ? "Paused" : "Ready to play?"}</div>
            <button
              className="mt-4 bg-blue-500 text-white px-4 py-2 rounded-md"
              onClick={() => {
                resume();
